...
void server_send_to_client(uint16_t dst_addr)
{
    ...
    _ctx.send_ttl = 3;
    _ctx.addr = dst_addr;            //gateway aka. client
    // _ctx.recv_dst = 0x000d;         // me. THIS SHOULD NOT NEEDED
    _ctx.srv_send = 0;

    // When an instance of a model is sending unsolicited msgs, it uses a model publish
    // addr as the destination addr.
    esp_err_t err = esp_ble_mesh_server_model_send_msg(onoff_server_0.model, &_ctx, ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS, sizeof(state.onoff), &state.onoff);
    if(err != ESP_OK){
                                         ...
                                         esp_ble_mesh_server_recv_gen_onoff_set_t *set)
{
    ...
    case ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK:
        ...
        // gpio_set_level(2, srv->state.onoff);
        break;
    default:
        break;
    }
                                               ...
                                               esp_ble_mesh_generic_server_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_GENERIC_SERVER_STATE_CHANGE_EVT:
        ...
        if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET ||
            ...
            // what is different here? why using auto-rsp?
...
esp_err_t ble_mesh_init(void)
{
    ...
    // Provisioning callback function in the BLE Mesh stack
    esp_ble_mesh_register_prov_callback(example_ble_mesh_provisioning_cb);
    esp_ble_mesh_register_config_server_callback(example_ble_mesh_config_server_cb);
    // --> Model operation callback function <--
    esp_ble_mesh_register_generic_server_callback(example_ble_mesh_generic_server_cb); 

    // Initialize BLE Mesh module
    ...
    // Enable the Advertising and Scan functions when the BLE Mesh initialization is completed. 
